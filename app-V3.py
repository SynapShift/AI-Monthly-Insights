...
def render_monthly_insights():
    ...
    with col3:
        st.markdown('<div class="filter-label">🔍 提示</div>', unsafe_allow_html=True)
        st.caption("数据实时同步自 Google Sheet")
    ...
    with c1:
        st.markdown(f'<div class="stat-card"><div class="stat-val">{len(df)}</div><div class="stat-label">📊 总计</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="stat-card"><div class="stat-val">{infra_n}</div><div class="stat-label">🏗️ 基建</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="stat-card"><div class="stat-val">{app_n}</div><div class="stat-label">🎨 应用</div></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="stat-card"><div class="stat-val">{fin_n}</div><div class="stat-label">💰 金融</div></div>', unsafe_allow_html=True)
        ...
        for idx, tab in enumerate(tabs):
            with tab:
                if idx == 0:
                    ...
                    cat_map = {1: "基建", 2: "应用", 3: "金融"}
                    subset = df[df[cat_col] == cat_map[idx]]
                render_feed_cards(subset)
    else:
        render_feed_cards(df)
