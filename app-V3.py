...
if GSHEET_URL:
    ...
    # 这里修改了标签为“选择月份”，但选中的内容依然是 sheet_name
    selected_sheet = st.sidebar.selectbox("📅 选择月份", all_sheets)
else:
...
if selected_sheet:
    df = load_data_from_gsheet(selected_sheet)
    ...
    if not df.empty:
        ...
        with tabs[1]: render_feed(df[df['分类'].str.contains('基建', na=False)]) if cat_col else st.warning("未找到分类列")
        with tabs[2]: render_feed(df[df['分类'].str.contains('应用', na=False)]) if cat_col else st.warning("未找到分类列")
        with tabs[3]: render_feed(df[df['分类'].str.contains('金融', na=False)]) if cat_col else st.warning("未找到分类列")
